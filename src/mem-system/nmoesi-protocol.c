...
void mod_handler_nmoesi_find_and_lock(int event, void *data)
{
 ...
 //ABM
            ...
            */
                ...
                int i = 0;
                int found_way = 0;
                struct mod_t* temp_l1;

                if(strstr(mod->name, "l2") && !stack->state)
                {
                    found_way =0 ;
                    //printf("Name = %s\n", mod->name);
                    for(i = 0 ; i < num_cores ;i++)
                    {
                        // Search only in L1 of upper
                        temp_l1 = list_get(mem_system->mod_list, i);
                      //  printf("ABM mod_name = %s\n", temp_l1->cache->name);
                        if(strstr(temp_l1->cache->name, "l1"))
                        {
                            //found_way = cache_find_block_head(temp_l1->cache, stack->addr);
                            //found_way = mod_find_block_head(temp_l1, stack->addr, &stack->set,
                            //        &stack->way, &stack->tag, &stack->state);
                            found_way = mod_find_block_head(temp_l1, stack->addr); 
                            if(found_way)
                            {
                                //printf("foundway = %d\n", found_way);
                                mod->cache->policy = cache_policy_qbs; // lets try with lru
                                mod->qbs_accesses++;
                                break;
                            }
                ...
                mod->cache->policy = cache_policy_lru;
            //ABM
			}
		}
		assert(stack->way >= 0);
                ...
                if(strstr(mod->name, "l2") && !stack->state)
                {
                    ...
                    for(i = 0 ; i < num_cores ;i++)
                    {
                        ...
                        if(strstr(temp_l1->cache->name, "l1"))
                        {
                            ...
                            {
                                //printf("foundway = %d\n", found_way);
                                mod->cache->policy = cache_policy_qbs;
                                mod->qbs_accesses++;
                                break;
                            }
