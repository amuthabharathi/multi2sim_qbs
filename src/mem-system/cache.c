	...
	struct cache_block_t *blk, enum cache_waylist_enum where)
{
	...
	else if (where == cache_waylist_tail)
	{
	...
	// ABM 
    /*
    else if (where = cache_waylist_tail_prev)
	{
		blk->way_prev = set->way_tail;
		blk->way_next = NULL;
		set->way_tail->way_next = blk;
...
//ABM
//Find in head
int cache_find_block_head(struct cache_t *cache, unsigned int addr)
{
	int set, tag, way;
    int found = 0;
	/* Locate block */
	...
	way = 0; // Search in head way alone
	for (way = 0; way < cache->assoc; way++)
    {
	if (cache->sets[set].blocks[way].tag == tag && cache->sets[set].blocks[way].state)
    {
        printf("In cache tag = %x and searching tag %x\n",cache->sets[set].blocks[way].tag, tag);
        found++;
...
{
	struct cache_block_t *head_block1;
    struct cache_block_t *tail_block1;
    struct cache_block_t *tail_block2;
	...
	*/
	...
	//ABM
    if (cache->policy == cache_policy_qbs && strstr(cache->name, "l2")) // If the requesting cache is LLC - L2
	{
        // Find in upper caches. If present in MRU do not evict it
        //printf("QBS triggered\n");
        //ABM - this is wrong because we havent moved the LRU to MRU
        // Make the LRU-1 block as LRU and perform LRU

        /************ Method - 1 *********
        tail_block3 = cache->sets[set].way_tail;
        tail_block2 = tail_block3->way_prev;
        tail_block1 = tail_block2->way_prev;

        tail_block1->way_next = tail_block3;
        tail_block3->way_prev = tail_block1;

        tail_block3->way_next = tail_block2;
        tail_block2->way_prev = tail_block3;

        tail_block2->way_next = NULL;
        cache->sets[set].way_tail = tail_block2;
        *
        */
        
        // | Head block | <--> ...... | tail_block1 | <--> | tail_block2 | --> NULL

        head_block1 = cache->sets[set].way_head;

        tail_block2 = cache->sets[set].way_tail;
        tail_block1 = tail_block2->way_prev;

        tail_block1->way_next = NULL; // Make LRU -1 as LRU

        tail_block2->way_prev = NULL; // This is becoming head
        tail_block2->way_next = head_block1; // This is becoming head
        head_block1->way_prev = tail_block2;


        cache->sets[set].way_tail = tail_block1;
        cache->sets[set].way_head = tail_block2;

		int way = cache->sets[set].way_tail->way_prev->way;
		cache_update_waylist(&cache->sets[set], cache->sets[set].way_tail, 
			cache_waylist_head);
